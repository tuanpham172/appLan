...
const CalendarHeader = ({ currentMonth, onPrev, onNext, onOpenSettings }: CalendarHeaderProps) => (
  <header className="flex flex-col sm:flex-row justify-between items-center mb-6 md:mb-10 gap-4">
    <div className="text-center sm:text-left w-full sm:w-auto">
      <h1 className="text-3xl md:text-4xl font-black tracking-tighter text-emerald-700">Lan Nguyễn</h1>
    </div>
    <div className="flex items-center gap-2 md:gap-4 w-full sm:w-auto justify-between sm:justify-end">
      <div className="flex items-center bg-white px-2 md:px-4 py-2 rounded-2xl shadow-sm border border-slate-200 grow sm:grow-0 justify-between">
        ...
        <span className="px-2 md:px-6 font-bold text-sm md:text-lg min-w-[120px] md:min-w-[160px] text-center text-slate-800 capitalize">{format(currentMonth, 'MMMM yyyy', { locale: vi })}</span>
...
const DayCell: React.FC<DayCellProps> = ({ 
  ...
  if (!isCurrentMonth) {
    return <div className="bg-slate-50/50 rounded-xl md:rounded-2xl border border-dashed border-slate-200 aspect-square" />;
  }
  ...
  return (
    <motion.div
      ...
      className={cn(
        "calendar-day relative flex flex-col items-center justify-center rounded-xl md:rounded-2xl transition-all duration-300 group shadow-sm",
    ...
    >
      <span className="text-lg md:text-2xl font-black">{format(date, 'd')}</span>
      
      {status === 'CHILL' && (
        <span className="absolute top-1 right-1 md:top-2 md:right-2 text-xs md:text-sm drop-shadow-sm">☕</span>
      )}
      
      <span className="text-[8px] md:text-[10px] opacity-80 uppercase font-black tracking-wider mt-0.5 md:mt-1">
        {status === 'CHILL' ? 'Đi làm' : 'Nghỉ'}
      </span>

      {isLocked && status && (
        <div className="absolute top-1 left-1 md:left-2 opacity-40">
          <X className="rotate-45" size={8} md:size={10} strokeWidth={4} />
        </div>
      )}
      
      {isNow && !status && (
        <div className="absolute top-1 left-1 md:top-2 md:left-2 w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-white animate-ping" />
      )}
    </motion.div>
  );
};
      ...
      >
        ...
        <div className="space-y-4">
          <button 
            ...
            className={cn(
              "w-full py-6 flex items-center justify-center gap-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all",
              currentStatus === 'CHILL' 
          ...
          {currentStatus && (
            <button 
              ...
              className="w-full py-5 flex items-center justify-center gap-3 rounded-2xl bg-rose-50 text-rose-500 hover:bg-rose-100 transition-all font-black text-xs uppercase tracking-widest mt-4"
            >
          ...
          <button 
            onClick={onClose}
            className="w-full py-4 flex items-center justify-center gap-3 rounded-2xl text-slate-400 hover:text-slate-600 transition-all font-black text-[10px] uppercase tracking-widest"
          >
            Đóng
          </button>
        </div>
      ...
      >
        ...
        <div className="space-y-6">
          ...
          <div className="pt-4 border-t border-slate-100 space-y-3">
            <button 
              onClick={() => {
                if (confirm('Bạn có chắc chắn muốn hủy công TOÀN BỘ các ngày trong tháng này?')) {
                  onResetMonth();
                  onClose();
                }
              }}
              className="w-full py-4 text-[10px] text-slate-500 font-black uppercase tracking-[0.2em] hover:bg-slate-50 rounded-xl transition-all border border-slate-100"
            >
              Hủy công toàn bộ tháng này
            </button>

            <button 
              onClick={() => {
                if (confirm('Bạn có chắc chắn muốn xóa TOÀN BỘ dữ liệu ứng dụng?')) {
                  onReset();
                  onClose();
                }
              }}
...
export default function App() {
  ...
  const handleLock = () => {
    if (confirm(`Xác nhận bạn đã nhận lương tháng ${format(currentMonth, 'MM/yyyy')}?\n\n- Số ngày làm: ${workedDays}\n- Số ngày nghỉ: ${offDays}\n- Tổng lương: ${totalSalary.toLocaleString()} đ\n\nSau khi xác nhận sẽ không thể chỉnh sửa.`)) {
      lockMonth({
  ...
  if (activeTab === 'report') {
    ...
    return (
      <div className="min-h-screen p-8 flex flex-col max-w-[1240px] mx-auto animate-in space-y-8">
        ...
        {/* Summary Card */}
          <div className="bg-emerald-900 text-white rounded-2xl md:rounded-3xl p-6 md:p-10 shadow-2xl shadow-emerald-900/40 relative overflow-hidden group transition-transform hover:scale-[1.01]">
          <div className="relative z-10">
            <p className="text-emerald-300 text-[10px] uppercase tracking-[0.4em] font-black mb-2 opacity-80">Tổng thu nhập đã nhận</p>
            <div className="text-3xl md:text-6xl font-black tabular-nums tracking-tighter">
              {totalReceivedSum.toLocaleString()} <span className="text-lg md:text-2xl text-emerald-400">đ</span>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
          {history.length > 0 ? history.map((data) => (
            <motion.div 
              key={data.monthStr}
              whileHover={{ y: -5 }}
              className="bg-white rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200 relative overflow-hidden"
            >
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center text-sm">
                  <span className="text-slate-500 font-bold tracking-tight">Ngày làm việc</span>
                  <span className="font-black text-emerald-600 text-lg tabular-nums">{data.count}</span>
                </div>
                <div className="flex justify-between items-center text-sm border-t border-slate-50 pt-3">
                  <span className="text-slate-500 font-bold tracking-tight">Ngày nghỉ</span>
                  <span className="font-black text-rose-500 text-lg tabular-nums">{data.offDays || 0}</span>
                </div>
                <div className="flex justify-between items-center text-sm border-t border-slate-100 pt-4">
                  <span className="text-slate-500 font-bold tracking-tight">Lương đã nhận</span>
  ...
  return (
    <div className="min-h-screen p-3 md:p-8 flex flex-col max-w-[1240px] mx-auto animate-in overflow-x-hidden">
      <CalendarHeader 
      ...
      <div className="flex flex-1 gap-6 md:gap-8 flex-col lg:flex-row">
        {/* Calendar Section */}
        <div className="flex-[2] bg-white rounded-2xl md:rounded-3xl shadow-sm border border-slate-200 p-4 md:p-8">
          <div className="grid grid-cols-7 gap-1 md:gap-4 text-center mb-4 md:mb-6">
            {weekDays.map((day, i) => (
              <div key={i} className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-widest">
                {day === 'S' && i === 5 ? 'T7' : day === 'S' && i === 6 ? 'CN' : `T${i + 2}`}
              </div>
            ))}
          </div>
        ...
        <div className="flex-1 flex flex-col gap-6">
          <div className="bg-white rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-sm border border-slate-200 relative overflow-hidden">
            {isMonthLocked && (
            ...
            <h2 className="text-lg md:text-xl font-black mb-6 md:mb-8 flex items-center text-slate-800 tracking-tight">
              <span className="mr-3 text-xl md:text-2xl">📊</span> Thống kê tháng này
            </h2>
            
            <div className="space-y-3 md:space-y-4">
              <div className="flex justify-between items-center p-3 md:p-4 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100">
                <span className="text-slate-500 text-[10px] md:text-xs font-black uppercase tracking-wider">Ngày làm việc</span>
                <span className="text-lg md:text-xl font-black text-emerald-600 tabular-nums">{workedDays}</span>
              </div>
              <div className="flex justify-between items-center p-3 md:p-4 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100">
                <span className="text-slate-500 text-[10px] md:text-xs font-black uppercase tracking-wider">Ngày không làm</span>
                <span className="text-lg md:text-xl font-black text-rose-500 tabular-nums">{offDays}</span>
              </div>
              <div className="flex justify-between items-center p-3 md:p-4 bg-slate-50 rounded-xl md:rounded-2xl border border-slate-100">
                <span className="text-slate-500 text-[10px] md:text-xs font-black uppercase tracking-wider">Đơn giá / ngày</span>
                <span className="text-xs md:text-sm font-black tabular-nums">{dailyRate.toLocaleString()} đ</span>
              </div>
            </div>
          </div>

          <div className="bg-emerald-900 text-white rounded-2xl md:rounded-3xl p-6 md:p-8 shadow-2xl shadow-emerald-900/40 relative group transition-transform hover:scale-[1.02]">
            <p className="text-emerald-300 text-[10px] uppercase tracking-[0.3em] font-black mb-1">Tổng lương dự kiến</p>
            <div className="text-3xl md:text-4xl font-black tabular-nums">{totalSalary.toLocaleString()} đ</div>
          </div>
          ...
          <div className="mt-4 lg:mt-auto space-y-4">
            <button 
              ...
              onClick={handleLock}
              className={cn(
                "w-full py-4 md:py-5 rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl transition-all",
                isMonthLocked 
      ...
      <footer className="mt-8 md:mt-12 flex justify-center sticky bottom-6 z-50">
        <div className="bg-white/90 backdrop-blur-md rounded-full px-6 md:px-12 py-3 md:py-4 shadow-2xl border border-slate-200 flex justify-between items-center w-[90%] max-w-sm">
          <div className={cn(
            "flex flex-col items-center cursor-pointer transition-all active:scale-90",
            activeTab === 'calendar' ? "text-emerald-600 scale-110" : "text-slate-400 hover:text-emerald-600"
          )} onClick={() => setActiveTab('calendar')}>
            <div className="w-5 h-5 md:w-6 md:h-6 mb-1 flex items-center justify-center">
            ...
            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-tighter sm:tracking-widest">Lịch công</span>
          </div>
          ...
          <div className={cn(
            "flex flex-col items-center cursor-pointer transition-all active:scale-90",
            activeTab === 'report' ? "text-emerald-600 scale-110" : "text-slate-400 hover:text-emerald-600"
            ...
            <span className="text-[9px] md:text-[10px] font-black uppercase tracking-tighter sm:tracking-widest">Báo cáo</span>
          </div>
        </div>
      </footer>
